...
public class AllLogReceiver {
    public static void main(String[] args) throws IOException, TimeoutException {
        ...
        // 绑定交换机
        channel.queueBind(queueName, LOGGER_V2,"info");
        channel.queueBind(queueName, LOGGER_V2,"warning");
        channel.queueBind(queueName, LOGGER_V2,"error");

        // 接收消息
        channel.basicConsume(queueName,(consumerTag, message) -> {
            System.out.println("All日志器，收到日志：" + new String(message.getBody(), StandardCharsets.UTF_8));
        },consumerTag -> {});
